...
class VotingSystem:
    ...
    def add_voter(self, voter_id, name, dob, phone):
        if len(voter_id) == 16:
            return "Voter ID must be 16 digits."
        if len(phone) != 10:
    ...
    def get_sorted_voter_ids(self):
        """Return a sorted list of voter IDs."""
        return sorted(self.voters.keys())

    def binary_search_voter(self, voter_id):
        """Perform binary search to find a voter by ID."""
        ...
        return None  # Voter ID not found
...
style.configure("Treeview", background="#282828", foreground="white", fieldbackground="#282828")
style.configure("Treeview.Heading", background="#565656", foreground="cyan")
...
def open_admin_panel():
    ...
    # Add search button to open a new search window
...
def send_otp_and_verify_edit(voter_id, phone):
    ...
    voter.otp = otp  # Store the OTP in the voter object

    # Display OTP in console for demonstration
    print(f"OTP for Voter ID {voter_id}: {otp}")
...
def select_candidate_and_vote(voter_id):
    ...
    # Initialize the candidate_var with a default value
    candidate_var = tk.StringVar(value="Select Candidate")

    # Create the OptionMenu with candidate_var as the variable
